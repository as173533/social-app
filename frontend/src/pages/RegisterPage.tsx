...
export function RegisterPage() {
  ...
  const validate = () => {
    ...
    if (form.name.trim().length < 2) nextErrors.push("Name must be at least 2 characters.");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) nextErrors.push("Enter a valid email address.");
    if (!/^\+?[0-9]{7,15}$/.test(form.phone.trim())) nextErrors.push("Phone number must contain 7 to 15 digits.");
    if (form.password.length < 8) nextErrors.push("Password must be at least 8 characters.");
    return nextErrors;
  };

  const apiErrors = (error: unknown) => {
    if (!axios.isAxiosError(error)) return ["Could not create account. Please try again."];
    const detail = error.response?.data?.detail;
    if (typeof detail === "string") {
      if (detail.toLowerCase().includes("email or phone")) return ["Email or phone number is already registered."];
      return [detail];
    }
      ...
      <form onSubmit={submit} className="w-full max-w-sm space-y-4">
        ...
        {(["name", "email", "phone", "password"] as const).map((field) => (
          <input
            ...
            className="w-full rounded-lg border border-slate-300 px-3 py-3 capitalize outline-none focus:border-mint"
            placeholder={field}
