...
const WHATSAPP_STYLE_EMOJI_GROUPS = {
    emoji: emojiItems([
    ...
    ], "emoji", "face smile happy laugh love sad angry emotion whatsapp recent"),
    gestures: emojiItems([
    ...
    ], "gesture", "hand gesture thumbs clap pray like dislike whatsapp"),
    people: emojiItems([
    ...
    ], "person", "people person family work face human whatsapp"),
    food: emojiItems([
    ...
    ], "food", "food drink fruit snack meal sweet whatsapp"),
    places: emojiItems([
    ...
    ], "place", "travel place car flight home office world whatsapp"),
    objects: emojiItems([
    ...
    ], "object", "object phone camera computer gift file work whatsapp"),
    symbols: emojiItems([
...
function requestSenderName(request) {
    return request.sender?.name ?? `User #${request.sender_id}`;
}
function requestSenderAvatar(request) {
...
function isCallEvent(message) {
    return message.body.startsWith("__call__:");
}
function formatCallEventTime(value) {
...
function callEventText(message) {
...
function messagePreview(message, currentUserId) {
    ...
    if (message.message_type === "image")
        return `${message.sender_id === currentUserId ? "You: " : ""}Photo`;
    if (message.message_type === "audio")
        return `${message.sender_id === currentUserId ? "You: " : ""}Voice message`;
    if (message.message_type === "video")
        return `${message.sender_id === currentUserId ? "You: " : ""}Video message`;
    if (message.message_type === "file")
        return `${message.sender_id === currentUserId ? "You: " : ""}${message.attachment_name ?? "Attachment"}`;
    if (message.message_type === "sticker")
        return `${message.sender_id === currentUserId ? "You: " : ""}Sticker`;
    if (message.message_type === "gif")
        return `${message.sender_id === currentUserId ? "You: " : ""}GIF`;
    return `${message.sender_id === currentUserId ? "You: " : ""}${message.body}`;
}
function messageUnread(message, currentUserId) {
    return Boolean(message && message.sender_id !== currentUserId && !includesNumber(message.read_by, currentUserId ?? 0));
}
function recorderMimeType(kind) {
...
export function MessengerPage() {
    ...
    const [emojiTab, setEmojiTab] = useState("all");
    ...
    const [videoMessageFacingMode, setVideoMessageFacingMode] = useState("user");
    ...
    const pendingSentIds = useMemo(() => new Set(requests.filter((request) => request.sender_id === user?.id && request.status === "pending").map((request) => request.receiver_id)), [requests, user?.id]);
    const pendingReceivedIds = useMemo(() => new Set(requests.filter((request) => request.receiver_id === user?.id && request.status === "pending").map((request) => request.sender_id)), [requests, user?.id]);
    const emojiGroups = {
        emoji: ["😮", "😢", "😠", "😍", "😎", "🙌", "🤝", "🔥", "🎉", "👀", "💯", "💡", "✅", "➕", "❌", "🔜", "🚢", "🤔", "🥺", "😭", "🤭", "😌", "😇", "👻", "😳", "😋", "😱", "😐", "🙄", "🤩", "🤪", "😏", "🥳", "🤣", "🫡", "🤗"],
        gestures: ["👍", "👎", "👌", "✌️", "🤞", "🤟", "🤘", "👋", "👏", "🙌", "🙏", "💪", "🫶", "☝️", "👊", "🤙"],
        people: ["😀", "😃", "😄", "😁", "🙂", "😊", "🥰", "😘", "😜", "🤓", "😴", "😷", "🤒", "🥶", "😵", "🤯"],
    ...
    const matchesSearch = (item) => !searchNeedle || `${item.label} ${item.keywords} ${item.value ?? ""} ${item.symbol ?? ""}`.toLowerCase().includes(searchNeedle);
    const filteredEmojiGroups = Object.fromEntries(Object.entries(pickerEmojiGroups).map(([category, items]) => [category, items.filter(matchesSearch)]));
    ...
    const formatDuration = (startedAt, tick = callTick) => {
        void tick;
        if (!startedAt)
            return "00:00";
    ...
    const conversationTitle = (conversation) => conversation?.conversation_type === "group"
    ...
    const loadAll = async () => {
        ...
        const conversationMessages = await Promise.all(conversationRows.map(async (conversation) => {
    ...
    const acceptIncomingOffer = async (fromUserId, sdp) => {
        const call = activeCallRef.current;
        if (call?.id && closedCallIds.current.has(call.id))
            return;
    ...
    useEffect(() => {
        if (!activeCall || activeCall.state !== "accepted")
            return;
    ...
    useEffect(() => {
        ...
        callTimeoutTimer.current = window.setTimeout(() => {
            ...
            if (!currentCall || currentCall.id !== activeCall.id || currentCall.state !== "ringing")
                return;
    ...
    useEffect(() => {
        ...
        rtc.current = new WebRTCClient((payload) => {
        ...
        const handleChatMessage = (event) => {
            ...
            if (payload.type === "friend_request:updated") {
                ...
                if (payload.status === "rejected") {
                    playMessageSound();
                    setRequestNotice("Your friend request was rejected.");
                    showActivityView();
                }
                if (payload.status === "accepted") {
                    playMessageSound();
                    setRequestNotice("Your friend request was accepted.");
                }
            }
            ...
            if (payload.type === "read") {
                ...
                setMessages((current) => current.map((message) => includesNumber(readMessageIds, message.id)
        ...
        const handleCallMessage = async (event) => {
            ...
            if (payload.type === "webrtc:offer") {
                if (!activeCallRef.current || closedCallIds.current.has(activeCallRef.current.id))
                    return;
                await acceptIncomingOffer(payload.from_user_id, payload.sdp).catch(() => setCallError("Could not connect the incoming audio stream. End the call and try again."));
            }
    ...
    useEffect(() => {
        const applyOutput = async () => {
            ...
            if (remote?.setSinkId)
                await remote.setSinkId(audioOutputId || "");
            if (audio?.setSinkId)
                await audio.setSinkId(audioOutputId || "");
            if (local?.setSinkId)
                await local.setSinkId(audioOutputId || "");
    ...
    const toggleGroupMember = (friendId) => {
        setGroupMemberIds((current) => current.includes(friendId) ? current.filter((id) => id !== friendId) : [...current, friendId]);
    };
    const startMessageSwipe = (message, event) => {
        messageSwipe.current = { id: message.id, x: event.clientX };
    };
    const endMessageSwipe = (message, event) => {
    ...
    const createGroup = async () => {
        ...
        if (!title || !groupMemberIds.length)
            return;
    ...
    const sendCallEventMessage = (callType, action) => {
        if (!selected)
            return;
        sendChatMessage({ body: `__call__:${callType}:${action}`, message_type: "call" });
    };
    const sendCallEndMessage = (call, state = "ended") => {
    ...
    const sendUploadedAttachment = async (attachment, caption = "") => {
        if (!selected)
            return;
        sendChatMessage({
            body: caption,
    ...
    const uploadAndSend = async (file, filename, caption = "") => {
        if (!selected)
            return;
        setUploading(true);
        setComposerError("");
    ...
    const sendQuickMessage = (value, messageType) => {
        sendChatMessage({ body: value, message_type: messageType });
        setShowEmojiPanel(false);
    };
    const startRecording = async (kind, facingMode = videoMessageFacingMode) => {
        setComposerError("");
        try {
    ...
    const getSearchAction = (result) => {
        if (friendIds.has(result.id))
            return { label: "Friends", disabled: true };
        if (pendingSentIds.has(result.id))
            return { label: "Request sent", disabled: true };
        if (pendingReceivedIds.has(result.id))
            return { label: "Respond", disabled: true };
    ...
    const deviceControls = (<div className="space-y-3">
      <label className="block text-xs font-medium text-slate-500">
        <span className="mb-1 flex items-center gap-1"><Mic size={14}/> Microphone</span>
        <select value={audioInputId} onChange={(event) => setAudioInputId(event.target.value)} onFocus={() => loadDevices().catch(() => undefined)} className="w-full rounded-md border border-[#d1d1e0] bg-white px-2 py-2 text-sm text-slate-800 outline-none focus:border-[#6264a7]">
          <option value="">Default microphone</option>
          {audioInputs.map((device, index) => (<option key={device.deviceId || index} value={device.deviceId}>
      ...
      <label className="block text-xs font-medium text-slate-500">
        ...
        <select value={videoInputId} onChange={(event) => setVideoInputId(event.target.value)} onFocus={() => loadDevices().catch(() => undefined)} className="w-full rounded-md border border-[#d1d1e0] bg-white px-2 py-2 text-sm text-slate-800 outline-none focus:border-[#6264a7]">
          <option value="">Default camera</option>
          {videoInputs.map((device, index) => (<option key={device.deviceId || index} value={device.deviceId}>
      ...
      <label className="block text-xs font-medium text-slate-500">
        ...
        <select value={audioOutputId} onChange={(event) => {
            setAudioOutputId(event.target.value);
            setSoundReady(false);
        }} onFocus={() => loadDevices().catch(() => undefined)} className="w-full rounded-md border border-[#d1d1e0] bg-white px-2 py-2 text-sm text-slate-800 outline-none focus:border-[#6264a7]">
          <option value="">Default speaker</option>
          {audioOutputs.map((device, index) => (<option key={device.deviceId || index} value={device.deviceId}>
              {device.label || `Speaker ${index + 1}`}
            </option>))}
        </select>
      </label>
      <button onClick={() => loadDevices().then(unlockAudio).catch(() => setCallError("Allow microphone/camera permission to show device names."))} className="w-full rounded-md border border-[#d1d1e0] py-2 text-sm font-medium hover:bg-[#f5f5fb]">
        Refresh devices
      </button>
      <button onClick={unlockAudio} className={`w-full rounded-md py-2 text-sm font-medium ${soundReady ? "bg-emerald-50 text-emerald-700" : "bg-[#6264a7] text-white"}`}>
    ...
    const callPanel = activeCall ? (activeCall.state === "ringing" && activeCall.callee_id === user?.id ? (<div className="overflow-hidden rounded-lg bg-[#4a403d] text-white shadow-2xl shadow-slate-900/35">
              ...
              <div className="relative">
                ...
                {showAudioMenu && (<div className="fixed inset-x-4 top-20 z-[90] max-h-[calc(100dvh-7rem)] overflow-y-auto rounded-md border border-[#ddddec] bg-white p-4 text-left text-sm text-slate-700 shadow-2xl sm:absolute sm:inset-x-auto sm:right-0 sm:top-14 sm:max-h-[min(70dvh,520px)] sm:w-80">
                    ...
                    <div className="space-y-2">
                      ...
                      {audioInputs.slice(0, 3).map((device) => (<label key={device.deviceId} className="flex items-center gap-2">
      ...
      {callMinimized ? (<button onClick={() => setCallMinimized(false)} className="mt-2 flex w-full items-center gap-3 text-left">
          ...
          {activeCall.state === "ringing" && activeCall.callee_id === user?.id && (<button onClick={() => setCallState("accepted")} className="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full bg-[#13a10e] px-6 py-3 font-semibold text-white shadow-lg hover:bg-[#0f7b0c]">
              <Phone size={18}/>Accept
            </button>)}
        </div>)}
    </div>)) : null;
    return (<main className="relative grid h-[calc(100dvh-3.5rem)] grid-cols-1 overflow-hidden bg-[#f5f5fb] pb-14 md:grid-cols-[64px_300px_minmax(0,1fr)] md:pb-0 xl:grid-cols-[64px_310px_minmax(0,1fr)_320px]">
      <nav className="fixed inset-x-0 bottom-0 z-40 grid h-14 grid-cols-3 justify-items-center border-t border-[#ddddec] bg-[#ebebf5] px-2 py-1 md:static md:h-auto md:grid-cols-1 md:border-r md:border-t-0 md:px-0 md:py-3 md:flex md:flex-col md:items-center md:gap-2">
        <button onClick={showChatView} className={`grid h-12 w-12 place-items-center rounded-md ${activeView === "chat" ? "bg-[#6264a7] text-white" : "text-[#464775] hover:bg-white"}`} title="Chat">
          <MessageSquare size={21}/>
        </button>
        <button onClick={showActivityView} className={`relative grid h-12 w-12 place-items-center rounded-md ${activeView === "activity" ? "bg-[#6264a7] text-white" : "text-[#464775] hover:bg-white"}`} title="Activity">
          <Bell size={21}/>
          {(incomingRequests.length > 0 || rejectedSentRequests.length > 0) && <span className="absolute right-2 top-2 h-2.5 w-2.5 rounded-full bg-[#c4314b]"/>}
        </button>
        <button onClick={showPeopleView} className={`grid h-12 w-12 place-items-center rounded-md ${activeView === "people" ? "bg-[#6264a7] text-white" : "text-[#464775] hover:bg-white"}`} title="People">
          <Users size={21}/>
        </button>
      </nav>
      ...
      <aside className={`${selected ? "hidden md:block" : "block"} overflow-y-auto border-r border-[#ddddec] bg-[#f7f7fc] p-3`}>
        <div className="mb-3 flex items-center justify-between px-1">
          ...
          <button type="button" onClick={() => activeView === "chat" ? setShowGroupModal(true) : activeView === "activity" ? showPeopleView() : showPeopleView()} className="grid h-9 w-9 place-items-center rounded-md bg-white text-[#464775] shadow-sm hover:bg-[#ededfa]" title={activeView === "chat" ? "Create group" : "People"}>
            {activeView === "chat" ? <MessageSquare size={17}/> : <UserPlus size={17}/>}
          </button>
        </div>
        {activeView === "activity" && (<div className="space-y-4">
            {requestNotice && (<div className="rounded-md border border-[#d1d1e0] bg-white p-3 text-sm text-slate-700">
                <div className="flex items-center justify-between gap-3">
                  <span>{requestNotice}</span>
                  <button type="button" onClick={() => setRequestNotice("")} className="text-slate-500 hover:text-slate-800">
                    <X size={15}/>
                  </button>
                </div>
              </div>)}
            <section className="rounded-md border border-[#d1d1e0] bg-white p-3">
              ...
              <div className="mt-3 space-y-2">
                ...
                {incomingRequests.map((request) => (<div key={request.id} className="rounded-md bg-[#f7f7fc] p-3">
            ...
            <section className="rounded-md border border-[#d1d1e0] bg-white p-3">
              ...
              <div className="mt-3 space-y-2">
                {rejectedSentRequests.length === 0 && <p className="text-sm text-slate-500">No request updates.</p>}
                {rejectedSentRequests.slice(0, 6).map((request) => (<div key={request.id} className="rounded-md bg-[#f7f7fc] p-3 text-sm">
                    <p className="font-medium">{request.receiver?.name ?? `User #${request.receiver_id}`}</p>
                    <p className="text-xs text-slate-500">rejected your friend request</p>
                  </div>))}
              </div>
            </section>
            <section className="rounded-md border border-[#d1d1e0] bg-white p-3">
              ...
              <div className="mt-3 space-y-2">
                ...
                return (<div key={call.id} className="rounded-md bg-[#f7f7fc] p-3 text-sm">
            ...
            <section className="rounded-md border border-[#d1d1e0] bg-white p-3">
              ...
              <div className="mt-3 space-y-2">
                ...
                {mentionMessages.slice(-6).map((message) => (<div key={message.id} className="rounded-md bg-[#f7f7fc] p-3 text-sm">
        ...
        <div className="relative">
          ...
          <input value={query} onChange={(event) => search(event.target.value)} className="w-full rounded-md border border-[#d1d1e0] bg-white py-2.5 pl-10 pr-3 outline-none focus:border-[#6264a7]" placeholder="Search"/>
        </div>
        ...
        <div className="mt-4 space-y-2">
          {results.map((result) => {
            ...
            return (<button key={result.id} onClick={() => !action.disabled && sendFriendRequest(result.id)} disabled={action.disabled} className="flex w-full items-center justify-between rounded-md border border-[#d1d1e0] bg-white p-3 text-left hover:bg-[#ededfa] disabled:cursor-default disabled:opacity-75 disabled:hover:bg-white">
        ...
        {activeView === "people" && (<>
            ...
            <div className="mt-3 space-y-2">
              ...
              {incomingRequests.map((request) => (<div key={request.id} className="rounded-md border border-[#d1d1e0] bg-white p-3">
        ...
        {activeView === "people" ? (<div className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="grid flex-1 gap-4 overflow-y-auto bg-[#f5f5fb] p-4 lg:grid-cols-2">
              <section className="rounded-md border border-[#d1d1e0] bg-white p-4">
                ...
                <div className="mt-3 space-y-2">
                  {incomingRequests.length === 0 && <p className="text-sm text-slate-500">No pending requests.</p>}
                  {incomingRequests.map((request) => (<div key={request.id} className="flex items-center justify-between rounded-md border border-[#d1d1e0] p-3">
              ...
              <section className="rounded-md border border-[#d1d1e0] bg-white p-4">
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {friends.map((friend) => (<button key={friend.friendship_id} onClick={() => openConversation(friend)} className="flex w-full items-center gap-3 rounded-md p-3 text-left hover:bg-[#ededfa]">
              ...
              {displayMessages.map((rawMessage) => {
                ...
                return (<div key={message.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                    <div onPointerDown={(event) => startMessageSwipe(message, event)} onPointerUp={(event) => endMessageSwipe(message, event)} onContextMenu={(event) => openMessageMenu(message, event)} className={`group relative max-w-[78%] rounded-md px-3.5 py-2 text-sm shadow-sm ${locallyUnreadIds.includes(message.id) ? "ring-2 ring-[#6264a7]/40" : ""} ${mine ? "bg-[#6264a7] text-white" : "bg-white text-slate-900"}`}>
                      ...
                      {message.message_type === "gif" && message.body.startsWith("http") && (<img src={message.body} alt="GIF" className="mb-2 max-h-72 rounded-md object-contain"/>)}
                      {message.deleted_for_everyone ? (<p className="italic opacity-80">This message was deleted</p>) : (<>
                      {message.message_type === "image" && message.attachment_url && (<img src={absoluteMediaUrl(message.attachment_url)} alt={message.attachment_name ?? "Image"} className="mb-2 max-h-72 rounded-md object-contain"/>)}
                      {message.message_type === "audio" && message.attachment_url && (<audio controls src={absoluteMediaUrl(message.attachment_url)} className="mb-2 max-w-full"/>)}
                      {message.message_type === "video" && message.attachment_url && (<video controls src={absoluteMediaUrl(message.attachment_url)} className="mb-2 max-h-80 max-w-full rounded-md bg-slate-950"/>)}
                      {message.message_type === "file" && message.attachment_url && (<a href={absoluteMediaUrl(message.attachment_url)} target="_blank" rel="noreferrer" className={`mb-2 flex items-center gap-2 rounded-md px-3 py-2 ${mine ? "bg-white/15 text-white" : "bg-slate-100 text-slate-800"}`}>
                          <FileUp size={16}/>
            ...
            <form onSubmit={sendMessage} className="relative border-t border-[#ddddec] bg-white p-3 sm:p-4">
              {showEmojiPanel && (<div className="absolute bottom-[68px] left-2 right-2 z-30 mx-auto flex h-[min(72dvh,540px)] max-w-[420px] flex-col overflow-hidden rounded-lg border border-[#d1d1e0] bg-white shadow-2xl sm:left-auto sm:right-3 sm:mx-0 sm:w-[390px]" onMouseDown={(event) => event.stopPropagation()}>
                  <div className="grid shrink-0 grid-cols-4 border-b border-[#e6e6f2] text-sm">
                    {["all", "emoji", "stickers", "gifs"].map((tab) => (<button key={tab} type="button" onClick={() => setEmojiTab(tab)} className={`py-3 font-medium capitalize ${emojiTab === tab ? "border-b-2 border-[#6264a7] text-slate-900" : "text-slate-500 hover:bg-[#f7f7fc]"}`}>
                        {tab === "gifs" ? "GIFs" : tab}
                      </button>))}
                  </div>
                  <div className="flex shrink-0 gap-1 overflow-x-auto border-b border-[#e6e6f2] px-2 py-2 text-lg">
                    ...
                    ["objects", "💡"],
                    ["symbols", "#️⃣"]
                ].map(([tab, icon]) => (<button key={tab} type="button" onClick={() => setEmojiTab(tab)} className={`grid h-8 w-8 shrink-0 place-items-center rounded-md ${emojiTab === tab ? "bg-[#ededfa] text-[#464775]" : "hover:bg-[#f7f7fc]"}`}>
                        {icon}
                      </button>))}
                  </div>
                  <div className="min-h-0 flex-1 overflow-y-auto p-3">
                    <div className="sticky top-0 z-10 bg-white pb-3">
                      ...
                      <input value={emojiSearch} onChange={(event) => setEmojiSearch(event.target.value)} className="w-full rounded-md border border-[#d1d1e0] py-2 pl-3 pr-9 text-sm outline-none focus:border-[#6264a7]" placeholder="Find emoji, GIF, sticker"/>
                    </div>
                    {emojiTab !== "gifs" && emojiTab !== "stickers" && (<>
                        ...
                        <div className="mt-2 grid grid-cols-8 gap-1">
                          {(emojiTab === "all" ? filteredEmojiOptions : filteredEmojiGroups[emojiTab]).map((emoji) => (<button key={`${emoji.symbol}-${emoji.label}`} type="button" onClick={() => setBody((value) => `${value}${emoji.symbol}`)} className="grid h-9 w-9 place-items-center rounded-md text-2xl leading-none hover:bg-[#ededfa]" title={emoji.label}>
                              {emoji.symbol}
                            </button>))}
                        </div>
                      </>)}
                    {(emojiTab === "all" || emojiTab === "stickers") && (<>
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2">
                          {filteredStickerOptions.map((sticker) => (<button key={sticker.label} type="button" onClick={() => sendQuickMessage(sticker.value, "sticker")} className={`grid aspect-square place-items-center rounded-lg ${sticker.color} text-4xl shadow-sm hover:scale-[1.02]`} title={sticker.label}>
                              {sticker.value}
                            </button>))}
                        </div>
                      </>)}
                    {(emojiTab === "all" || emojiTab === "gifs") && (<>
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2">
                          {giphyResults.length > 0 ? giphyResults.map((gif) => (<button key={gif.id} type="button" onClick={() => sendQuickMessage(gif.url, "gif")} className="overflow-hidden rounded-md bg-slate-100 shadow-sm hover:scale-[1.02]" title={gif.title}>
                              <img src={gif.preview} alt={gif.title} className="aspect-square w-full object-cover"/>
                            </button>)) : filteredGifOptions.map((gif, index) => (<button key={gif.value} type="button" onClick={() => sendQuickMessage(gif.value, "gif")} className={`grid aspect-square place-items-center rounded-md p-2 text-center text-xs font-semibold text-white shadow-sm hover:scale-[1.02] ${index % 3 === 0 ? "bg-[#6264a7]" : index % 3 === 1 ? "bg-[#0f766e]" : "bg-[#c4314b]"}`}>
      ...
      <aside className="hidden overflow-y-auto border-l border-[#ddddec] bg-[#f7f7fc] p-4 xl:block">
        ...
        <div className="mt-3 space-y-2">
          {incomingRequests.map((request) => (<div key={request.id} className="flex items-center justify-between rounded-md border border-[#d1d1e0] bg-white p-3">
      ...
      {showGroupModal && (<div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/50 px-4">
          <section className="w-full max-w-md rounded-lg bg-white shadow-2xl">
            ...
            <div className="space-y-4 p-4">
              <input value={groupTitle} onChange={(event) => setGroupTitle(event.target.value)} className="w-full rounded-md border border-[#d1d1e0] px-3 py-2 outline-none focus:border-[#6264a7]" placeholder="Group name"/>
              <div className="max-h-72 space-y-2 overflow-y-auto">
                {friends.map((friend) => (<label key={friend.friendship_id} className="flex cursor-pointer items-center gap-3 rounded-md p-2 hover:bg-[#f5f5fb]">
                    <input type="checkbox" checked={groupMemberIds.includes(friend.user.id)} onChange={() => toggleGroupMember(friend.user.id)} className="accent-[#6264a7]"/>
                    <span className="grid h-9 w-9 place-items-center overflow-hidden rounded-full bg-[#6264a7] font-semibold text-white">
                      {friend.user.avatar ? <img src={avatarSrc(friend.user.avatar)} alt={friend.user.name} className="h-full w-full object-cover"/> : friend.user.name.slice(0, 1).toUpperCase()}
                    </span>
              ...
              <button type="button" onClick={createGroup} disabled={!groupTitle.trim() || !groupMemberIds.length} className="w-full rounded-md bg-[#6264a7] py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">
                Create group
              </button>
            </div>
          </section>
      ...
      {recordingPreview && (<div className="fixed inset-0 z-[90] grid place-items-center bg-slate-950/60 px-4">
          <section className="w-full max-w-lg overflow-hidden rounded-lg bg-white shadow-2xl">
            ...
            <div className={recordingPreview.kind === "audio" ? "bg-white p-5" : "bg-slate-950"}>
              {recordingPreview.kind === "audio" ? (<div className="rounded-md border border-[#ddddec] bg-[#f8f8fd] p-4">
                  ...
                  {recordingPreview.blob ? (<audio src={recordingPreview.url} controls className="w-full"/>) : (<p className="text-sm text-slate-500">Recording audio...</p>)}
                </div>) : recordingPreview.stream ? (<div className="relative">
                  <video ref={recordingPreviewVideo} muted autoPlay playsInline className="aspect-video w-full object-cover"/>
                  <button type="button" onClick={switchVideoMessageCamera} className="absolute right-3 top-3 rounded-md bg-slate-950/70 px-3 py-2 text-xs font-semibold text-white hover:bg-slate-900">
                    {videoMessageFacingMode === "user" ? "Back camera" : "Front camera"}
                  </button>
                </div>) : (<video key={recordingPreview.url} src={recordingPreview.url} controls playsInline className="aspect-video w-full object-cover"/>)}
            </div>
            <div className="flex flex-wrap justify-end gap-2 p-4">
              {recordingKind ? (<button onClick={() => recorder.current?.stop()} className="flex items-center gap-2 rounded-md bg-[#c4314b] px-4 py-2 font-medium text-white">
